...
layout(location=2) flat in int inOffset;
layout(location=0) out float outColor;

layout(binding=0) readonly buffer GlyphBufferBlock
{
	GlyphPoint points[];
} pointBuffer;
...
void main()
{
	...
	int  state      = 0;
	int  point_idx  = inOffset;
	vec2 frag_coord = inScale * inFragCoord;
	...
	do {
		...
		do {
			gp = pointBuffer.points[point_idx++];
			...
			case 1: // previous point is on-curve
				if (bool(gp.flags & FLAG_ON_CURVE)) {
					p1 = gp.position;
					line(min_dist, p0, p1, frag_coord);
			...
			case 2: // previous point is off-curve
				if (bool(gp.flags & FLAG_ON_CURVE)) {
					p2 = gp.position;
					quadratic(min_dist, p0, p1, p2, frag_coord);
		...
		} while (!bool(gp.flags & FLAG_END_CONTOUR));

		if (state == 1)
			line(min_dist, p0, ps, frag_coord);
		else
			quadratic(min_dist, p0, p1, ps, frag_coord);
				
	} while (!bool(gp.flags & FLAG_END_GLYPH));

	outColor = min_dist.distance;
}
