...
{
public:
	VERA_CONSTEXPR AABB2D() VERA_NOEXCEPT :
		m_min(FLT_MAX, FLT_MAX),
		m_max(-FLT_MAX, -FLT_MAX) {}

	VERA_CONSTEXPR AABB2D(float min_x, float min_y, float max_x, float max_y) VERA_NOEXCEPT :
		m_min(min_x, min_y),
		m_max(max_x, max_y)
	{
		VERA_ASSERT(m_min.x <= m_max.x && m_min.y <= m_max.y);
	}

	VERA_CONSTEXPR AABB2D(const float2& min, const float2& max) VERA_NOEXCEPT :
		m_min(min),
		m_max(max)
	{
		VERA_ASSERT(m_min.x <= m_max.x && m_min.y <= m_max.y);
	...
	VERA_NODISCARD VERA_CONSTEXPR const float2& min() const VERA_NOEXCEPT
	{
		return m_min;
	}

	VERA_NODISCARD VERA_CONSTEXPR const float2& max() const VERA_NOEXCEPT
	{
		return m_max;
	}

	VERA_NODISCARD VERA_CONSTEXPR float2 center() const VERA_NOEXCEPT
	{
		return (m_min + m_max) * 0.5f;
	}

	VERA_NODISCARD VERA_CONSTEXPR float2 size() const VERA_NOEXCEPT
	{
		return m_max - m_min;
	}

	VERA_NODISCARD VERA_CONSTEXPR bool intersect(const AABB2D& rhs) const VERA_NOEXCEPT
	{
		return 
	...
	VERA_NODISCARD VERA_CONSTEXPR bool contain(const float2& p) const VERA_NOEXCEPT
	{
		return
	...
	VERA_NODISCARD VERA_CONSTEXPR bool contain(const AABB2D& rhs) const VERA_NOEXCEPT
	{
		return
	...
	VERA_CONSTEXPR void expand(const float2& point) VERA_NOEXCEPT
	{
	...
	VERA_CONSTEXPR void expand(const AABB2D& aabb) VERA_NOEXCEPT
	{
	...
	VERA_NODISCARD VERA_CONSTEXPR bool operator==(const AABB2D& rhs) const VERA_NOEXCEPT
	{
