...
class matrix_base<3, 4, row_major, T, Q>
{
public:
	...
	using this_type   = matrix_base<3, 4, row_major, T, Q>;

	static VERA_CONSTEXPR mat3x4_type identity() VERA_NOEXCEPT
	{
		return { static_cast<T>(1) };
	}

	static VERA_CONSTEXPR mat3x4_type zero() VERA_NOEXCEPT
	{
	...
	VERA_NODISCARD VERA_CONSTEXPR mat4x3_type transpose() const VERA_NOEXCEPT
	{
		return {
			rows[0][0], rows[1][0], rows[2][0], rows[0][1], rows[1][1], rows[2][1], rows[0][2], rows[1][2], rows[2][2], rows[0][3], rows[1][3], rows[2][3]
		};
	}
	...
	VERA_NODISCARD VERA_CONSTEXPR mat2x4_type operator*(const mat2x3_type& rhs) const VERA_NOEXCEPT
	{
		return {
	...
	VERA_NODISCARD VERA_CONSTEXPR mat3x4_type operator*(const mat3x3_type& rhs) const VERA_NOEXCEPT
	{
		return {
			rows[0][0] * rhs[0] + rows[0][1] * rhs[1] + rows[0][2] * rhs[2],
			rows[1][0] * rhs[0] + rows[1][1] * rhs[1] + rows[1][2] * rhs[2],
	...
	VERA_NODISCARD VERA_CONSTEXPR mat4x4_type operator*(const mat4x3_type& rhs) const VERA_NOEXCEPT
	{
		return {
			rows[0][0] * rhs[0] + rows[0][1] * rhs[1] + rows[0][2] * rhs[2],
			rows[1][0] * rhs[0] + rows[1][1] * rhs[1] + rows[1][2] * rhs[2],
	...
	VERA_CONSTEXPR VERA_NODISCARD matrix_base operator/(T rhs) const VERA_NOEXCEPT
	{
		return {
			rows[0] / rhs,
			rows[1] / rhs,
...
template <class T, MathQualifier Q>
class matrix_base<3, 4, col_major, T, Q>
{
public:
	...
	using this_type   = matrix_base<3, 4, col_major, T, Q>;
