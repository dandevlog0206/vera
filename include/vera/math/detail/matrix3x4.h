...
public:
	...
	using this_type   = matrix_base<3, 4, T, Q>;
	...
	VERA_CONSTEXPR matrix_base(
		...
		T m20, T m21, T m22, T m23) VERA_NOEXCEPT :
		col{
	...
	VERA_CONSTEXPR matrix_base(
		const col_type& col0,
		const col_type& col1,
		const col_type& col2) VERA_NOEXCEPT :
		col{
			col0,
			col1,
			col2 } {}
	...
	VERA_NODISCARD VERA_CONSTEXPR vec4_type operator*(const vec3_type& rhs) const VERA_NOEXCEPT
	{
		return {
			...
			col[0][3] * rhs[0] + col[1][3] * rhs[1] + col[2][3] * rhs[2]
		};
	}
