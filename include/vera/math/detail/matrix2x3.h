...
class matrix_base<2, 3, row_major, T, Q>
{
public:
	...
	using this_type   = matrix_base<2, 3, row_major, T, Q>;

	static VERA_CONSTEXPR mat2x3_type identity() VERA_NOEXCEPT
	{
		return { static_cast<T>(1) };
	}

	static VERA_CONSTEXPR mat2x3_type zero() VERA_NOEXCEPT
	{
	...
	VERA_NODISCARD VERA_CONSTEXPR mat3x2_type transpose() const VERA_NOEXCEPT
	{
		return {
			rows[0][0], rows[1][0],
			rows[0][1], rows[1][1],
	...
	VERA_NODISCARD VERA_CONSTEXPR mat2x2_type operator*(const mat3x2_type& rhs) const VERA_NOEXCEPT
	{
		return {
	...
	VERA_NODISCARD VERA_CONSTEXPR mat2x3_type operator*(const mat3x3_type& rhs) const VERA_NOEXCEPT
	{
		return {
			rows[0][0] * rhs[0] + rows[0][1] * rhs[1] + rows[0][2] * rhs[2],
			rows[1][0] * rhs[0] + rows[1][1] * rhs[1] + rows[1][2] * rhs[2]
	...
	VERA_NODISCARD VERA_CONSTEXPR mat2x4_type operator*(const mat3x4_type& rhs) const VERA_NOEXCEPT
	{
		return {
			rows[0][0] * rhs[0] + rows[0][1] * rhs[1] + rows[0][2] * rhs[2],
			rows[1][0] * rhs[0] + rows[1][1] * rhs[1] + rows[1][2] * rhs[2]
...
template <class T, MathQualifier Q>
class matrix_base<2, 3, col_major, T, Q>
{
public:
	...
	using this_type   = matrix_base<2, 3, col_major, T, Q>;
